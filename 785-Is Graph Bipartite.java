...
class Solution {
    ...
    public boolean isBipartite(int[][] graph) {
        ...
        for (int i=0; i<n; i++){
            if (colors[i] == -1 && !dfs(colors, graph, i, 0)) return false; 
        }
        return true;
    }
    
    public boolean dfs(int[] colors, int[][] graph, int index, int color){
        if (colors[index] != -1) return colors[index]==color;
        
        colors[index] = color;
        for(int nei:graph[index]){ 
            if (!dfs(colors, graph, nei, 1 - color)) return false;
        }
