...
class Solution {
    ...
    //deal with stack of scopes(inner expresion will inherit parent scope)
    private int eval(String exp, Map<String, Integer> parent) {
        if (exp.charAt(0) != '(') {
            // just a number or a symbol
            if (Character.isDigit(exp.charAt(0)) || exp.charAt(0) == '-') //deal with negative number
                return Integer.parseInt(exp);
            return parent.get(exp);
        }
        
        Map<String, Integer> map = new HashMap<>(); // create a new scope
        map.putAll(parent); //copy parent scope to it
        ...
        if (exp.startsWith("(a")) { // add
            ...
            for (int i = 0; i < tokens.size() - 2; i += 2)
                map.put(tokens.get(i), eval(tokens.get(i + 1), map)); //let x 2
            return eval(tokens.get(tokens.size() - 1), map);
        }
    }
    
    // seperate the values between two parentheses
    // consider case: let x (add 3 2) y (add 1 2) -> let, x, add 3 2, y, add 1 2
